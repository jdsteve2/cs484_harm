...
*************************************************************************/
// Changing #define to extern values
extern int GlobalN1;
extern int GlobalN2;
extern int N1;
extern int N2;
...
extern double (** a_p)[NPR];
extern double (** a_dq)[NPR];
extern double (** a_F1)[NPR];
extern double (** a_F2)[NPR];
extern double (** a_ph)[NPR];
extern int **a_pflag;
...
extern double (** a_conn)[NDIM][NDIM][NDIM];
extern double (** a_gcon)[NPG][NDIM][NDIM];
extern double (** a_gcov)[NPG][NDIM][NDIM];
extern double (** a_gdet)[NPG];
...
#define I_Alloc_2D(x, d1, d2) \
	x = malloc(sizeof(int *) * (d1)); \
	x[0] = malloc(sizeof(int) * (d1) * (d2)); \
	for (i = 0; i < (d1); i++ ) \
		x[i] = &x[0][i*(d2)];

#define D_Alloc_1D(x, d1) \
	x = malloc(sizeof(double) * (d1));

#define D_Alloc_2D(x, d1, d2) \
	x = malloc(sizeof(double *) * (d1)); \
	x[0] = malloc(sizeof(double) * (d1) * (d2)); \
	for (i = 0; i < (d1); i++ ) \
		x[i] = &x[0][i*(d2)];

#define D_Alloc_2D_1(x, d1, d2, d3) \
	x = malloc(sizeof(double *) * (d1)); \
	x[0] = malloc(sizeof(double[d3]) * (d1) * (d2)); \
	for (i = 1; i < (d1); i++ ) \
		x[i] = &(x[0][i*(d2)]); 
	
#define D_Alloc_2D_3(x, d1, d2, d3, d4, d5) \
	x = malloc(sizeof(double *) * (d1)); \
	x[0] = malloc(sizeof(double[d3][d4][d5]) * (d1) * (d2)); \
	for (i = 0; i < (d1); i++ ) \
		x[i] = &x[0][i*(d2)]; 

#define AD_Alloc_2D(x, y, d1, xoff, yoff, tmp) \
    tmp = malloc(sizeof(double *) * (d1)); \
	for (i = 0; i < (d1); i++ ) \
        tmp[i] = &(y[i][yoff]); \
    x = &(tmp[xoff]);

#define AI_Alloc_2D(x, y, d1, xoff, yoff, tmp) \
    tmp = malloc(sizeof(int *) * (d1)); \
	for (i = 0; i < (d1); i++ ) \
        tmp[i] = &y[i][yoff]; \
    x = &tmp[xoff];
